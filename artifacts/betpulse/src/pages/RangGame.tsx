...
const CSS = `
@keyframes rng3DReveal {
  0%   { transform:rotateY(90deg) scale(.8); opacity:.3; }
  100% { transform:rotateY(0deg) scale(1); opacity:1; }
}
@keyframes trumpReveal {
  0%   { transform:scale(0) rotate(-22deg) translateY(20px); opacity:0; }
  55%  { transform:scale(1.2) rotate(4deg) translateY(-4px); opacity:1; }
  80%  { transform:scale(.97) rotate(-2deg); }
  100% { transform:scale(1) rotate(0); opacity:1; }
}
@keyframes trumpPulse {
  0%,100% { filter:drop-shadow(0 0 14px currentColor); }
  50%      { filter:drop-shadow(0 0 32px currentColor) drop-shadow(0 0 50px currentColor); }
}
...
@keyframes goldGlow { 0%,100%{box-shadow:0 0 16px rgba(251,191,36,.6),2px 4px 0 #92400e} 50%{box-shadow:0 0 36px rgba(251,191,36,1),2px 4px 0 #92400e} }
`;
...
function playTrumpReveal() {
  try { const c=mkAudio();[330,415,523,659,784].forEach((f,i)=>{const o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);o.type="triangle";o.frequency.value=f;const t=c.currentTime+i*.12;g.gain.setValueAtTime(.22,t);g.gain.exponentialRampToValueAtTime(.001,t+.4);o.start(t);o.stop(t+.4);});setTimeout(()=>c.close(),2500);}catch(_){}
}
function playTrickWin() {
  try { const c=mkAudio();[440,554].forEach((f,i)=>{const o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);o.type="sine";o.frequency.value=f;const t=c.currentTime+i*.08;g.gain.setValueAtTime(.14,t);g.gain.exponentialRampToValueAtTime(.001,t+.2);o.start(t);o.stop(t+.2);});setTimeout(()=>c.close(),1000);}catch(_){}
}
function playWin() {
...
function Confetti({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    const COLS = ["#fbbf24","#f59e0b","#a78bfa","#fff","#4ade80","#f472b6"];
...
function PlayCard({ card, revealed, delay=0, idx=0, total=5, trumpSuit="", isWinner=false }: {
  card: CardObj; revealed: boolean; delay?: number; idx?: number; total?: number; trumpSuit?: string; isWinner?: boolean;
}) {
  ...
  const spread = total > 1 ? (idx / (total - 1) - 0.5) * 26 : 0;
  const liftY = Math.abs(spread) * 0.38;
      ...
      <div style={{ width:52, height:76, perspective:600 }}>
        <div style={{
          ...
          <div style={{
            ...
            borderRadius:7,
            background: isTrump ? "linear-gradient(150deg,#fffbeb,#fef3c7)" : "linear-gradient(150deg,#ffffff,#f5f0e8)",
            border:`2px solid ${isTrump?"#f59e0b":isCourt?"#d97706":"rgba(0,0,0,.16)"}`,
            boxShadow: isTrump
              ? "0 0 18px rgba(245,158,11,.7), 2px 4px 0 #92400e"
              : "2px 4px 0 rgba(0,0,0,.32), 4px 8px 0 rgba(0,0,0,.15)",
            animation: isTrump && revealed ? "goldGlow 1.1s ease-in-out infinite" : undefined,
            overflow:"hidden",
          }}>
            ...
            {isTrump && <div style={{position:"absolute",bottom:14,left:0,right:0,textAlign:"center",fontSize:6,fontWeight:900,color:"#b45309",letterSpacing:.5}}>RANG</div>}
...
function TrumpBadge({ suit, revealed }: { suit: string; revealed: boolean }) {
  ...
  return (
    <div style={{
      ...
      background:`radial-gradient(ellipse at center,${m.glow.replace(".8",".18")} 0%,transparent 70%)`,
      border:`3px solid ${m.color}`,
      boxShadow:`0 0 40px ${m.glow},inset 0 0 20px ${m.glow.replace(".8",".07")}`,
      animation: revealed ? "trumpReveal .7s cubic-bezier(.36,.07,.19,.97) both" : undefined,
    }}>
      <div style={{ fontSize:50, lineHeight:1, color:m.color, animation:revealed?"trumpPulse 2s ease-in-out infinite":undefined }}>{suit}</div>
      <div style={{ fontSize:10, fontWeight:900, letterSpacing:3, color:m.color }}>RANG · {m.label.toUpperCase()}</div>
...
function TrickRow({ trick, idx, show }: { trick: TrickResult; idx: number; show: boolean }) {
  ...
  const pFace = IS_RED(trick.playerCard.suit) ? "#dc2626" : "#1a2744";
  const hFace = IS_RED(trick.houseCard.suit)  ? "#dc2626" : "#1a2744";
  return (
    <div style={{display:"flex",alignItems:"center",gap:8,justifyContent:"center",animation:"trickSlide .3s ease-out both",animationDelay:`${idx*.1}s`}}>
      <div style={{width:40,height:56,borderRadius:6,background:"linear-gradient(150deg,#fff,#f5f0e8)",border:`2px solid ${pw?"#fbbf24":"rgba(0,0,0,.18)"}`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",boxShadow:pw?"0 0 14px rgba(251,191,36,.7), 2px 3px 0 #92400e":"2px 3px 0 rgba(0,0,0,.3)"}}>
        <div style={{fontSize:12,fontWeight:900,color:pFace,fontFamily:"Georgia,serif"}}>{trick.playerCard.rank}</div>
        <div style={{fontSize:11,color:pFace}}>{trick.playerCard.suit}</div>
      </div>
      <div style={{fontSize:9,fontWeight:900,color:pw?"#fbbf24":hw?"#f87171":"rgba(255,255,255,.28)",letterSpacing:1,textAlign:"center",minWidth:28}}>
        {pw?"▶WIN":hw?"LOSE◀":"DRAW"}
      </div>
      <div style={{width:40,height:56,borderRadius:6,background:"linear-gradient(150deg,#fff,#f5f0e8)",border:`2px solid ${hw?"#f87171":"rgba(0,0,0,.18)"}`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",boxShadow:hw?"0 0 14px rgba(248,113,113,.6), 2px 3px 0 rgba(0,0,0,.3)":"2px 3px 0 rgba(0,0,0,.3)"}}>
        <div style={{fontSize:12,fontWeight:900,color:hFace,fontFamily:"Georgia,serif"}}>{trick.houseCard.rank}</div>
        <div style={{fontSize:11,color:hFace}}>{trick.houseCard.suit}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{position:"relative",width:50,height:50}}>
      <svg width={50} height={50} style={{position:"absolute",inset:0,transform:"rotate(-90deg)"}}>
        ...
        <circle cx={25} cy={25} r={r} stroke={s<=3?"#ef4444":"#fbbf24"} strokeWidth={3} fill="none" strokeDasharray={circ} strokeDashoffset={circ*(1-s/30)} style={{transition:"stroke-dashoffset .9s linear,stroke .3s"}}/>
      </svg>
...
const EMPTY_HAND: CardObj[] = [{rank:"A",suit:"♠",value:14,isTrump:false},{rank:"K",suit:"♥",value:13,isTrump:false},{rank:"Q",suit:"♦",value:12,isTrump:false},{rank:"J",suit:"♣",value:11,isTrump:false},{rank:"10",suit:"♠",value:10,isTrump:false}];
...
export default function RangGame() {
  ...
  const handleDeal = async () => {
    ...
    if (cdRef.current) clearInterval(cdRef.current);
    clearTmr(); setPhase("dealing"); setResult(null); setShowConfetti(false); setShowWin(false); setShowTrump(false); setVisibleTricks(0);
    setRevP([false,false,false,false,false]); setRevH([false,false,false,false,false]);
    try {
      const resp = await fetch("/api/games/rang",{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({stake,selection})});
      ...
      const pollId:ReturnType<typeof setInterval>=setInterval(async()=>{
        ...
        try {
          ...
          for(let i=0;i<5;i++){
            addTmr(()=>{playFlip();setRevP(rv=>{const n=[...rv];n[i]=true;return n;});},1000+i*260);
            addTmr(()=>{playFlip();setRevH(rv=>{const n=[...rv];n[i]=true;return n;});},1000+i*260+130);
          }
          const trickStart=1000+4*260+130+450;
          setPhase("tricks");
          for(let t=0;t<(data.tricks?.length??0);t++){
            addTmr(()=>{setVisibleTricks(t+1);playTrickWin();},trickStart+t*500);
          }
          addTmr(()=>{
          ...
          },trickStart+(data.tricks?.length??0)*500+350);
        }catch{}
      },500);
    }catch{toast({title:"Network Error",variant:"destructive"});setPhase("betting");}
  };

  const handleAgain=()=>{clearTmr();setPhase("betting");setSelection(null);setStake(0);setCustomStake("");setResult(null);setShowConfetti(false);setShowWin(false);setShowTrump(false);setVisibleTricks(0);setRevP([false,false,false,false,false]);setRevH([false,false,false,false,false]);};
  ...
  if (isLoading) return <div style={{display:"flex",height:"100vh",alignItems:"center",justifyContent:"center",background:"#050510"}}><div style={{width:44,height:44,borderRadius:"50%",border:"4px solid #fbbf24",borderTopColor:"transparent",animation:"spin .8s linear infinite"}}/></div>;

  return (
    <div style={{minHeight:"100vh",background:"radial-gradient(ellipse at 50% 0%,#1a1000 0%,#050510 60%,#0a0800 100%)",overflowX:"hidden"}}>
      <style>{CSS}</style>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"11px 16px",background:"rgba(0,0,0,.6)",backdropFilter:"blur(14px)",borderBottom:"1px solid rgba(255,255,255,.07)",position:"sticky",top:0,zIndex:30}}>
        ...
        <div style={{textAlign:"center"}}>
          <div style={{fontSize:16,fontWeight:900,letterSpacing:4,color:"white",fontFamily:"Georgia,serif"}}>🃏 RUNG</div>
          <div style={{fontSize:9,color:"rgba(255,255,255,.35)",letterSpacing:2}}>TRUMP CARD · BEST OF 5 TRICKS</div>
        </div>
      ...
      <div style={{maxWidth:540,margin:"0 auto",padding:"12px 10px 24px"}}>
        ...
        <div style={{perspective:1000,perspectiveOrigin:"50% -10%",marginBottom:12}}>
          <div style={{
            ...
            border:`5px solid ${trumpMeta?trumpMeta.color+"88":"#4a3000"}`,
            borderRadius:22,
            ...
            <div style={{display:"flex",justifyContent:"center",marginBottom:14}}>
              {!showTrump&&phase==="betting"&&(
                <div style={{padding:"10px 22px",borderRadius:16,border:"2px dashed rgba(251,191,36,.28)",background:"rgba(251,191,36,.04)",textAlign:"center"}}>
                  <div style={{fontSize:32,opacity:.28}}>🃏</div>
                  <div style={{fontSize:10,color:"rgba(255,255,255,.28)",letterSpacing:2}}>RANG HIDDEN</div>
                </div>
              )}
              {!showTrump&&phase!=="betting"&&(
                <div style={{padding:"10px 22px",borderRadius:16,border:"2px solid rgba(251,191,36,.4)",background:"rgba(251,191,36,.08)",textAlign:"center"}}>
                  <div style={{fontSize:10,color:"#fbbf24",letterSpacing:2,fontWeight:900,animation:"dealing .7s ease-in-out infinite"}}>REVEALING RANG...</div>
                </div>
              )}
                ...
                <div style={{display:"flex",justifyContent:"center",alignItems:"flex-end",height:86}}>
                  {playerHand.map((card,i)=>(
                    <div key={i} style={{marginLeft:i===0?0:-16}}>
                      <PlayCard card={card} revealed={phase==="betting"?false:(revP[i]??false)} delay={i*.15} idx={i} total={playerHand.length} trumpSuit={trumpSuit}/>
                    </div>
                  ))}
                </div>
                {revP.every(Boolean)&&result&&(
                  <div style={{marginTop:6,fontSize:10,color:"#fbbf24",fontWeight:900}}>
                    🔥 {result.playerHand?.filter((c:any)=>c.isTrump).length??0} trump · {playerTricks} tricks
                  </div>
                )}
              </div>

              {/* VS */}
              <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",paddingTop:26,minWidth:40,gap:6}}>
              ...
              <div style={{flex:1,textAlign:"center"}}>
                ...
                <div style={{display:"flex",justifyContent:"center",alignItems:"flex-end",height:86}}>
                  {houseHand.map((card,i)=>(
                    <div key={i} style={{marginLeft:i===0?0:-16}}>
                      <PlayCard card={card} revealed={phase==="betting"?false:(revH[i]??false)} delay={i*.15} idx={i} total={houseHand.length} trumpSuit={trumpSuit}/>
                    </div>
                  ))}
                </div>
                {revH.every(Boolean)&&result&&(
                  <div style={{marginTop:6,fontSize:10,color:"#fbbf24",fontWeight:900}}>
                    🔥 {result.houseHand?.filter((c:any)=>c.isTrump).length??0} trump · {houseTricks} tricks
                  </div>
                )}
              </div>
            </div>
            ...
            {(phase==="tricks"||phase==="result")&&result?.tricks&&(
              <div style={{marginTop:12}}>
                <div style={{fontSize:9,color:"rgba(255,255,255,.35)",letterSpacing:2,textAlign:"center",marginBottom:6}}>TRICK RESULTS</div>
            ...
            {/* Win pop */}
            {showWin&&result?.won&&(
              <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",textAlign:"center",zIndex:30,animation:"rngWin .5s cubic-bezier(.36,.07,.19,.97) both"}}>
            ...
            <div style={{textAlign:"center",marginTop:12,minHeight:30}}>
              {phase==="betting"&&<p style={{color:"rgba(255,255,255,.25)",fontSize:12,letterSpacing:2}}>PICK A SIDE AND DEAL</p>}
              {phase==="dealing"&&<p style={{color:"#4ade80",fontSize:13,fontWeight:900,letterSpacing:4,animation:"dealing .6s ease-in-out infinite"}}>DEALING...</p>}
        ...
        <div style={{background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)",borderRadius:18,padding:16}}>
          {phase==="result"?(
            ...
            <>
              ...
              <div style={{display:"flex",gap:7,marginBottom:11,justifyContent:"center"}}>
                {CHIPS.map(amt=>(
                  <button key={amt} onClick={()=>phase==="betting"&&setStake(amt)} disabled={phase!=="betting"}
                    style={{width:50,height:50,borderRadius:"50%",border:`3px solid ${stake===amt?"#fbbf24":"rgba(255,255,255,.18)"}`,background:stake===amt?"radial-gradient(circle at 38% 35%,#fde68a,#fbbf24,#d97706)":"radial-gradient(circle at 38% 35%,#374151,#1f2937)",color:stake===amt?"#78350f":"rgba(255,255,255,.45)",fontWeight:900,fontSize:11,cursor:"pointer",boxShadow:stake===amt?"0 0 16px rgba(251,191,36,.6),inset 0 2px 0 rgba(255,255,255,.4)":"inset 0 2px 0 rgba(255,255,255,.06)",transition:"all .2s"}}>
              ...
              {!isAuthenticated
                ?<button onClick={()=>setLocation("/login")} style={{width:"100%",padding:13,borderRadius:12,background:"linear-gradient(135deg,#fbbf24,#f59e0b)",color:"#78350f",fontWeight:900,fontSize:15,border:"none",cursor:"pointer"}}>LOG IN TO PLAY</button>
                :<>
                  <button onClick={handleDeal} disabled={!canDeal} style={{width:"100%",padding:14,borderRadius:12,fontWeight:900,fontSize:16,letterSpacing:3,background:canDeal?"linear-gradient(135deg,#fbbf24,#f59e0b)":"rgba(255,255,255,.06)",color:canDeal?"#78350f":"rgba(255,255,255,.22)",border:`2px solid ${canDeal?"rgba(251,191,36,.6)":"rgba(255,255,255,.06)"}`,cursor:canDeal?"pointer":"not-allowed",boxShadow:canDeal?"0 4px 24px rgba(251,191,36,.4)":"none",transition:"all .2s"}}>
